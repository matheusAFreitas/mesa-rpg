  ...
  render() {
    ...
    el.innerHTML = [...App.db.session].reverse().map(s => `
      <div class="sitem">
        <div class="shead">
          <div>
            <div class="snum">Sessão ${s.num} — ${s.date}</div>
            <div class="stitle">${s.title||'(sem título)'}</div>
          </div>
        ...
        ${s.summary ? `<div class="snotes">${s.summary}</div>` : ''}
        ${s.npcs_met ? `<div style="font-size:10px;color:var(--text2);margin-top:6px;">👥 ${s.npcs_met}</div>` : ''}
        ${s.locations ? `<div style="font-size:10px;color:var(--text2);">📍 ${s.locations}</div>` : ''}
      </div>`).join('');
  }
};
